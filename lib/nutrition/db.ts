...
export const FOOD_SELECT =
  "id, name, brand, barcode, source, off_id, serving_size_g, serving_unit, servings, " +
...
export const LOG_SELECT =
  "id, food_id, meal_group_id, date, food_name, brand, quantity_g, serving_label, " +
 ...
 * a hardcoded uid, but a different user would otherwise leave an
 * empty list — this guards that.
 */
export async function ensureMealGroups(
  ...
  const rows = DEFAULT_GROUPS.map((g) => ({ ...g }));
  const { data: seeded } = await supabase
    .from("meal_groups")
    .upsert(rows, { onConflict: "space_id,name", ignoreDuplicates: true })
    .select("id, name, position, created_at");
