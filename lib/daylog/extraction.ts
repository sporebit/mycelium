...
export const EXTRACTION_TOOL = {
  ...
  input_schema: {
    ...
    properties: {
      ...
      open_thread: { type: "string", description: "one line naming something the narrator left unresolved or pointed at a later date — going back next week, waiting to hear, a plan, an appointment, a decision pending. Record it whenever the exchange contains one; at most one." },
      quotes: {
        type: "array",
...
export const EXTRACTION_SYSTEM = [
  ...
  "Rules: record only what the narrator said — never world knowledge, never guesses about places or people. A scene is a distinct place-or-activity segment of the day, in the order it happened; reuse an existing scene's ref to add people or detail to it. The narrator (I / me / we) is never listed as a person and is never a fact subject. A fact about another person (a preference, something that happened to them) takes that person's name as subject and kind person_fact unless food, drink or spend fits better. Money goes in a spend fact with data.amount_pence when a figure was given. A first or a turning point in a relationship or a life is a milestone, and milestone outranks every other kind (she posted a photo of us for the first time: a milestone, not media). A name that is not in the known-people list and has not been explained goes in unknown_names; when the narrator explains who someone is, put it in name_answers. Pets are fine as people entries. Whenever the narrator reports someone's exact words (she said, he goes, quote marks, or I said), record those words under quotes with the speaker's name — in addition to any fact they imply, never instead of it. Whenever the narrator points at something still to come or unresolved (we are going back next week, waiting on the results, have not decided yet), put it in open_thread as one line — a later night will ask how it went. Use confidence inferred only when you are reading between the lines.",
].join("\n");
