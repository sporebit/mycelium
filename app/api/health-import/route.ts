...
export async function POST(req: NextRequest) {
  ...
  // Public route, no session: act as the user bound to the health import
  // integration in configuration, never one named by the request.
  return withUser(boundUser("health_import"), async (supabase) => {
    ...
    if (body.metrics && Array.isArray(body.metrics)) {
      const rows = body.metrics
        ...
        .map((m) => {
          ...
          return { metric_type: (m.type || m.metric_type)!,
            value: m.value,
            unit: m.unit || "count",
      ...
      if (rows.length > 0) {
        ...
        for (let i = 0; i < rows.length; i += batchSize) {
          ...
          const { error } = await supabase
            .from("health_metrics")
            .upsert(batch, {
              onConflict: "space_id,metric_type,date,source",
    ...
    if (body.workouts && Array.isArray(body.workouts)) {
      const rows = body.workouts
        ...
        .map((w) => {
          ...
          return { workout_type: (w.type || w.workout_type)!,
            start_at: startAt,
      ...
      if (rows.length > 0) {
        ...
        for (let i = 0; i < rows.length; i += batchSize) {
          ...
          const { error } = await supabase
            ...
            .upsert(batch, {
              onConflict: "space_id,workout_type,start_at",
              ignoreDuplicates: false,
            });
          if (error) {
...
export async function GET(req: NextRequest) {
  // The whole prefix is public (PUBLIC_PREFIXES); the secret is the gate
  // for reads too, or an anonymous caller could learn Phil's sync times.
  const bearer = req.headers.get("authorization")?.replace("Bearer ", "");
  const secret = process.env.HEALTH_IMPORT_SECRET;
  if (!secret || bearer !== secret) {
    return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  ...
  try {
    const data = await withUser(boundUser("health_import"), async (supabase) => {
