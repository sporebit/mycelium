...
## P12 — MULTI-USER / TEAMS (supersedes P10) 🔶 PARTS 0–6 DONE ON BRANCH `multi-user` — PART 7 (CUTOVER) IS PHIL'S SESSION
...
| P12 | 🔶 In progress — Parts 0–6 done; Part 7 (cutover) is Phil's session | 9988ac3 (docs), 80d02a0 (drift fix), d08869a (Part 0), 05d5cf2 (Part 1), b2aebbc (Part 2), 1e3bc36 (Part 3), b6e60bf (Part 4), Part 5 = feat(security) commit, Part 6 = feat(rundowns) commit on `multi-user` | Branch from `b9892d1`. Locked: Docker + `supabase start`; legacy HS256 JWT secret EXISTS (Part 3 mints short-lived JWTs); Phil's auth uid is FIXED (`app.legacy_user_uid('phil')` = `lib/system/identity.ts`, Part 7 must create his live auth user with that id). Part 0 (VERIFY 0, 10 Sep): chain 0001–0101 replays clean after guarded fixes to 0001/0003/0055/0097; registry + coverage test; rollback runbook. Part 1 (VERIFY 1, 10 Sep): migration `0102_profiles.sql` (profiles, app schema, legacy uid mapping, my_sessions()); `@supabase/ssr` user + browser clients; service client fenced to lib/system/** by ESLint with a deprecated shim at the old path; new middleware (public → pc-metrics → system secrets → session refresh → break-glass → aal2 for /admin) with unspoofable x-principal headers; /login (magic link, password, passkey, Google, TOTP step); Settings → Security (password, TOTP, passkeys, sessions); break-glass keyed on BREAK_GLASS_SECRET, 404 when disabled, never reaches /admin. 59 automated checks green (33 HTTP flag-off, 18 Playwright incl. passkey via virtual authenticator, 8 HTTP flag-on); Google untested (needs provider config). Part 2 (VERIFY 2 half 1, 10 Sep): `0103_spaces.sql` (teams scaffold, spaces, entity_groups seeded from the registry, app.personal_space(), profile→space trigger, `app.adopt_table()` helper, shared-reference read policies) + `0104`–`0109` adopting all 85 registered tables (space_id NOT NULL + FK + index, created_by uuid, user_id-keyed PK/unique/indexes rebuilt on space_id, legacy policies dropped, user_id dropped; STOP raised on any unmappable user_id). `scripts/verify-ownership.ts` (`npm run verify:ownership`): from-empty replay 0 failures over 85 tables, 209 rows preserved; PostgREST probes as Phil/anon correct; tests 100/100. Decision to confirm: the `user_settings` `'default'` placeholder from 0075 is deleted when a real row exists and untouched, adopted as Phil's when alone, STOP if dirty. **VERIFY 2 half 2 (live dump replay) blocked on checklist §4.** Part 3 (VERIFY 3, 10 Sep): `0110_access.sql` (team_members, team_member_sections, user_grants with finance+platform excluded, app.accessible_spaces/visible_spaces) + `0111_policies.sql` (four permissive policies per registered table generated over entity_groups, owner-only shape for finance AND platform, grants to authenticated, space_id/created_by column defaults, SQL functions lose p_user_id); lib/system/{jwt,withUser,bindings,admin}; createUserClient honours the middleware principal (session / API_SECRET / break-glass); 277 files swapped by codemod + six agents, `lib/supabase/server.ts` deleted, 0 `process.env.USER_ID` left; `scripts/isolation-test.ts` (`npm run isolation-test`): 138 endpoints × 2 users, 0 leaks, 0 foreign rows over 85 tables, Phil's counts intact; 15 policy tests; tests 117/117; build clean. Found and fixed: PayPal sync importing Phil's payments into another user's space (now instance-owner-only), health-import GET unauthenticated, fail-open crons, user-agnostic calendar cache. Part 4 (VERIFY 4, 10 Sep): `0112_teams_invites.sql` (invites with sha256 tokens/7-day/single-use, invite-only trigger on auth.users, SECURITY DEFINER functions for every team/membership/grant/invite operation, requires_totp()); 0110's recursive team_members policy fixed via app.is_team_member(); API routes for teams/members/invites/grants/lookup; Settings → People & teams; /invite/[token] landing with magic link/password/Google; Resend wrapper with local link fallback; mandatory TOTP hand-off for owners/admins. 27 matrix + flow tests (role×section×verb, toggles, grants, finance refused, one owner, successor, invite expiry/single use, invite-only), 15 Playwright checks (invite → new account → first screen with hidden sections), isolation gate clean (143 endpoints). Part 5 (VERIFY 5, 10 Sep): `0113_audit.sql` (append-only audit_events written by triggers on auth.sessions/auth.mfa_factors/invites/team_members/team_member_sections/teams/user_grants plus app.audit(); profiles.disabled_at with personal_space()/accessible_spaces() returning nothing for disabled users; end_session(); delete_my_account()/app.delete_user(); admin_* reads over access tables only; admin feature flags confined to three user_settings columns) + `0114_rate_limits.sql` (Postgres token bucket; ten failed second factors in fifteen minutes lock); re-auth cookie (ten minutes, HMAC on SUPABASE_JWT_SECRET) required with aal2 on /admin, /api/admin, /api/account; rate-limited /api/auth/password, magic-link, mfa/verify, invites; remote sign-out; 'who has seen my data'; export zip of the personal space by entity group; account deletion with typed confirmation; /admin console; cross-user reads audited from 71 list routes (one row per foreign space per request); break-glass requests audited via createUserClient. Found and fixed by the driver: a trailing-slash prefix let a first-factor session delete an account. 9 DB/static tests, 31+1 HTTP checks. Part 6 (VERIFY 6, 10 Sep): `0115_rundowns.sql` (settings per team, subscriptions per member, issues per recipient/week/channel); renderer runs under withUser(recipient) so RLS shapes each copy; `/api/cron/rundowns` hourly (CRON_SECRET; owner may force); email via Resend, push via the recipient's own subscriptions, in-app `/rundowns/[team]/[week]`, Telegram only for Phil; owner/member controls on People & teams. 3 render tests (different toggles → different issues, opted-out gets none, idempotent), 11 HTTP checks (non-member 404). Tests 156/156, isolation gate clean over 152 endpoints, build clean. **Open (Phil): Part 2 dump replay (§4); checklist §1 (auth providers, redirect URL, magic-link template → token_hash) and §3 (BREAK_GLASS_*, SUPABASE_JWT_SECRET) untouched.** nutrition_targets → health.nutrition confirmed by Phil 10 Sep. §1/§3/§4 deferred by Phil 10 Sep. Read `docs/multi-user-handoff.md` first — say "get started" |
