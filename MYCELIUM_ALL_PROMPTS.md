...
## P12 — MULTI-USER / TEAMS (supersedes P10) 🔶 IN PROGRESS — PART 0, BRANCH `multi-user`
...
| P12 | 🔶 In progress — Part 0 | (docs commit on `multi-user`) | Branch created from `b9892d1`. Locked: Docker + `supabase start` as the rehearsal env; legacy HS256 JWT secret EXISTS, so Part 3 mints short-lived JWTs rather than using the direct-Postgres fallback. Registry analysis done — 92 live relations = 91 tables + 1 view; the prompt covers 90 tables once the journal/places/reminders/events/daily_logs/memory_chunks shorthand is expanded. `pc_metrics_machines` is a `security_invoker` VIEW, so it needs no `space_id` and the coverage test must exclude views. **One open STOP item: `nutrition_targets` (created by 0098, after the prompt was written) needs an entity group — recommend `health.nutrition`.** Outstanding: deps, `supabase init` (no config.toml exists), replay 0001–0101, registry.ts, rollback doc. Build gate owed on the docs commit |
