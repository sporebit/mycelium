...
## P12 — MULTI-USER / TEAMS (supersedes P10) 🔶 IN PROGRESS — PART 0 DONE, PART 1 NEXT, BRANCH `multi-user`
...
| P12 | 🔶 In progress — Part 0 done, Part 1 next | 9988ac3 (docs), 80d02a0 (drift fix), d08869a (Part 0) on `multi-user` | Branch created from `b9892d1`. Locked: Docker + `supabase start` as the rehearsal env; legacy HS256 JWT secret EXISTS, so Part 3 mints short-lived JWTs rather than using the direct-Postgres fallback. VERIFY 0 passed 10 Sep 2026: chain 0001–0101 replays clean from empty after guarded fixes to 0001/0003/0055/0097 (all no-ops on live); `lib/access/registry.ts` covers 85 entity tables in 25 groups + 6 shared-reference + 1 derived view, proven by `lib/access/registry.test.ts` against the local stack; rollback runbook and Phil's checklist written; build clean; branch pushed. New migrations start at **0102** (main already carries 0098–0101). **One open STOP item for Part 2: `nutrition_targets` placed in `health.nutrition` pending Phil's one-word confirmation.** Part 3 caveats found: `events` has no `user_id` (backfill `created_by` from the space owner); there is no `api_usage` table in public. Read `docs/multi-user-handoff.md` first — say "get started" |
