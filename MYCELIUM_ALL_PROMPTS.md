...
| P7 | 🔶 Parts 1+3+4 done, 2 partial | 916e9c4, 65865bc | Numerals/hierarchy done, incl. a privacy fix: chart tickFormatters leaked amounts while finance was hidden, now routed through formatGBP({hidden}). 180 token replacements. Part 2: SpendingClient on useApi (transactions keyed by filter combo); Accounts/Investments/Analysis clients still raw fetch |
| P8 | 🔶 Parts 1-3 done, 4 partial | 1eb5d90, f3e09c8 | Drops/Studio/The Boys swept (160 replacements). Settings v2 partial: Appearance density + motion + dashboard-layout reset, Sections multi-select, hidden_sections now also filters ⌘K. Phil's decisions: ONE flag system (hidden_sections), hidden = nav + ⌘K only with URLs still resolving, Export/API-Usage restyle in scope. Still to do: left-anchored section nav, mobile Sheet nav, Integrations/Capture/Data restyle |
