...
**2026-09-15 (cutover executed, direct route):** Phil ran the P12 DB cutover live from the PC. `main` fast-forwarded to the merged branch and `supabase db push` applied 0102–0115 to the hosted project after a forward fix: 0104/0111 recreated `search_memory_chunks` with a bare pgvector `<=>` that the hosted push search_path could not resolve, fixed by qualifying `OPERATOR(extensions.<=>)` (commit `839f43f`). First push had committed only 0102–0103 (0104 rolled back, no data touched). Post-push: hosted at 0115, one profile/instance owner/personal space, zero orphaned `space_id` over the sampled tables. App-level verification and legacy-env retirement still pending. hosted migration state is now **0115**, not 0101.
