...
### 12.3 Part D as built (2026-09-22) `[claude]`

Shipped as `f9d6b65` `ee75a6f` `eacc0bd` `a72a758` `b8a0b8d` then the fixes `0d70842` `dead96f` `31135f4`, migration **0132** (the private `daylog` bucket). Production smoke 22/22 on the final deploy (photos from the page, the media routes, the week block, the memory chunk, the carry-over on a second night; two test days, cleaned up after). Phil's live check is the `smoke-test-daylog-part-d` template (MYC-144) — it wants a real evening for the Telegram photo path and a second night for the carry-over.

- **Seeds** (`lib/daylog/seeds.ts`): calendar (the day's events from the shared feeds), weather (`weather_cache`, the day's row), Spotify (`spotify_plays` for the day, top artists), media (`media_items` whose status changed that day). **Apple Health is a stub** returning nothing until the import exists (§4.2 says degrade silently). Gathered by the cron at prompt time and stored on `daylog_days.seeds`; a day started from the page gathers them then. The prompt reads "Evening. Calendar had X · dry, 17°. Talk, Quick or Skip?"; the interviewer gets a labelled block in the (cached) system prompt, marked as not what happened.
- **Photos** (`lib/daylog/media.ts`, bucket 0132): a Telegram photo while the day is open or awaiting scores is the day's, caption or not (`/c ` in the caption forces the ticket path); a caption is also run as a message. The page has + PHOTO. **Scene attach is by timing**: each scene's span starts where its title words first appear in the transcript; a photo goes to the latest scene that had started when it arrived (before all → first, after all → last). Runs at close, only fills nulls; `PATCH /api/journal/media/[id]` moves one by hand. Signed URLs, one hour.
- **Carry-over** (decision 24) — **deviation: it rides on the opener, not on the model.** `startTalk` takes the most recent earlier day's unasked `open_thread`, marks it `open_thread_asked_at`, and opens with "Last time you mentioned … How did that go? Then tell me about today." Rule 7 tells the model to ask once more only if the first message did not answer it. A first version put the thread in the system prompt for one turn and the model chose a slot probe instead — a fixed opener costs nothing and cannot forget.
- **Memory index**: at close the summary + approved facts become one `memory_chunks` row, `source_type = daylog_day`, replacing the day's previous chunk; `/api/ask` surfaces it as an "other" source. `source_id` has no FK, so `DELETE /api/journal/days/[date]` removes the chunk and the bucket objects itself.
- **Monthly alert** (decision 23): `monthlyAlert` on every cron tick and at close; once per month, tracked as `user_settings.daylog.alerted_month` (no new table; written through `updateUserSettings`).
- **Agent context diet**: `recentDaysContext` — the last three closed days' first lines, ≤ ~300 words — appended to the system prompt of Da Boi and of whichever agent is `persona_agent_id`. Read on each turn, never written to `agent_memory` (flag 6).
- **Weekly review**: `GET /api/journal/week?week=2026-W38` and a Day log block under the Tickets block on `/review` — the week's days, first lines, score dots, averages, logged/skipped counts.
- **Turn cap** was already enforced in Part A; nothing new.
- Also from this part's smokes: narrative bullets now match scenes by content words when the counts differ (`assignSceneLines`), so a five-bullet narrative for four scenes no longer leaves every scene without a line.

### 12.4 Part E as built (2026-09-22) `[claude]`

Shipped as `f020a4d` `0ef6e28` `d61b2c6` (+ the template commit), migration **0133**. **The five parts are now live; the day log is complete per §12.** Phil's live check is the `smoke-test-daylog-part-e` template (MYC-146) — it needs a second account in a team.

- **Consent is a team** (flag 3): `app.shares_team_with(uid)` (SECURITY DEFINER) and a `before insert or update of linked_user_id` trigger on `people` refuse any link to a user who shares no team with the caller (`42501`). The route checks first and maps the error to 403; the trigger is the wall. `linked_at` is stamped and cleared by the same trigger.
- **The viewer's read path is one function**, `public.daylog_shared_scenes(p_from, p_to)` (SECURITY DEFINER, rpc): scenes where `app.daylog_scene_visible_to_linked(id)` holds and the day is someone else's, with the **date**, the owner's display name, the participants' names and the shareable photos. That is the whole of decision 30. The viewer still cannot read `daylog_days` or `daylog_facts` at all — the function is the only way a date reaches them. §3's per-table policies remain from 0130 for the direct-table reads (scenes, scene_people, shareable media).
- **Routes**: `GET/POST/DELETE /api/people/[id]/link` (GET also lists the caller's teammates as candidates); `GET /api/journal/shared?from=&to=`. **UI**: LINK TO A TEAMMATE / UNLINK in the person page's Days header; SHARED WITH LINKED / HIDDEN FROM LINKED on each scene card; private / shared in each photo's corner; a "With <owner>" card on the viewer's own day page.
- **The cross-user test exists and passes** — `lib/access/daylogLinked.test.ts`, 9 checks through PostgREST on the local stack as Phil, Tess (a teammate) and an outsider: the outsider cannot be linked; before the link Tess sees nothing; after it she sees exactly her one unhidden scene with its date and the participants' names, never the day row, the fact, the private photo or the hidden scene's membership; she cannot write; unlink removes everything at once. `npm test` is 236/236 and the isolation script is **0 leaks over 185 endpoints × 2 users and 120 tables** (Docker was started for this; the 4 Phil-side 5xx are Google/Spotify without local OAuth creds, as on 09-11).
- Found by the local stack, not by tsc: `position` is a reserved word in a `returns table` list — the output column is `scene_position`.
- **Deviations**: the `hidden_from_linked` control is a toggle on the chip row, not in the edit form; a person's "linked" state is shown in the Days block rather than the identity column. Participants' names come from the owner's People rows via the function (the spec's "else display the name text only" fallback is not needed — the function always has the row).
