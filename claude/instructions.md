...
**Start of every conversation.** Read `claude/context.md` and `claude/backlog.md` before touching the request; read or search other project docs as the task requires.
...
**Backlog.** `claude/backlog.md` is canonical: one doc, sections per stream, statuses idea / committed / in-progress / blocked / done. Passing ideas ("we could…") are auto-captured as `idea`; Claude may add self-spotted items tagged `[claude]`. No fixed priority order. Done = builds, tests pass, Phil has verified it live. Open bugs are queue items, not gates.
...
| `claude/backlog.md` | Canonical backlog, all streams (becomes a generated export when Tickets ships) |
